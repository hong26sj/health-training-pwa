import {demoHealthSnapshot,normalizeHealthSnapshot} from './health-model.js';
import {deriveDashboardFromHealth} from './health-engine.js';

const DEMO_KEY='healthTrainingDemoData';
const GAS_URL_KEY='healthTrainingGasUrl';

const defaultDemo=deriveDashboardFromHealth(demoHealthSnapshot,{
...
function ensureShape(data){
  let d=data||clone(defaultDemo);
  if(!d.healthRaw)d.healthRaw=clone(demoHealthSnapshot);
  d=deriveDashboardFromHealth(normalizeHealthSnapshot(d.healthRaw),d);
  ...
  if(!d.strength)d.strength=clone(defaultDemo.strength);
  if(!Array.isArray(d.strength.recent))d.strength.recent=[];
...
export function getHealthSnapshot(data){return normalizeHealthSnapshot(ensureShape(data).healthRaw);}
export function applyHealthSnapshot(data,snapshot){const d=ensureShape(data);d.healthRaw=normalizeHealthSnapshot(snapshot);const next=ensureShape(d);saveLocalDashboard(next);return next;}
